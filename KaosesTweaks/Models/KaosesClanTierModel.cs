...
namespace KaosesTweaks.Models
{
    class KaosesClanTierModel : DefaultClanTierModel
    {
        ...
        public override int GetPartyLimitForTier(Clan clan, int clanTierToCheck)
        {
            ...
            KaosesMCMSettings settings = KaosesMCMSettings.Instance ?? throw new InvalidOperationException();
            if (settings.ClanAdditionalPartyLimitEnabled && clan == Clan.PlayerClan && settings.ClanPlayerPartiesLimitEnabled)
            {
                result.Add((float)(settings.ClanPlayerBasePartiesLimit + Math.Floor(clanTierToCheck * settings.ClanPlayerPartiesBonusPerClanTier)), new TextObject("KT Player Clan Parties Tweak"));
            }
            else if (settings.ClanAIPartiesLimitTweakEnabled && clan.IsClan && !clan.StringId.Contains("_deserters"))
            {
                ...
                {
                    result.Add((float)(settings.BaseAICustomSpawnPartiesLimit + Math.Floor(clanTierToCheck * settings.ClanCSPartiesBonusPerClanTier)), new TextObject("KT Custom Spawn Parties Tweak"));

                }
                else if (settings.ClanAIMinorClanPartiesLimitTweakEnabled && clan.IsMinorFaction && !clan.StringId.StartsWith("cs_"))
                {
                    result.Add(base.GetPartyLimitForTier(clan, clanTierToCheck) + settings.ClanAIBaseClanPartiesLimit, new TextObject("KT Minor Clan Parties Tweak"));
                }
                else if (clan.IsClan)
                {
                    result.Add((float)(settings.ClanAIBaseClanPartiesLimit + Math.Floor(clanTierToCheck * settings.ClanAIPartiesBonusPerClanTier)), new TextObject("KT AI Clan Parties Tweak"));
                }
            }
        ...
        public override int GetCompanionLimit(Clan clan)
        {
            ...
            if (settings.ClanCompanionLimitEnabled)
            {
        ...
        {
            if (KaosesMCMSettings.Instance is {ClanCompanionLimitEnabled: true})
            {
                return clanTier + KaosesMCMSettings.Instance.ClanCompanionBaseLimit;
            }
            return clanTier + 3;
        }
        ...
        public override ValueTuple<ExplainedNumber, bool> HasUpcomingTier(Clan clan, out TextObject extraExplanation, bool includeDescriptions = false)
        {
            ...
            if (flag)
            {
                ...
                {
                    item.Add(1f, _mercenaryEligibleText);
                }
                if (clan.Tier + 1 == VassalEligibleTier)
                {
                    item.Add(1f, _vassalEligibleText);
                }
                if (clan.Tier + 1 == KingdomEligibleTier)
                {
                    item.Add(1f, _kingdomEligibleText);
                }
            }
