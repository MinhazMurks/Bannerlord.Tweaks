...
namespace Tweaks.Behaviors
{
	...
	{
		public override void RegisterEvents() => CampaignEvents.OnSessionLaunchedEvent.AddNonSerializedListener(this, new Action<CampaignGameStarter>(this.OnSessionLaunched));
		...
		{
			if (Romance.GetRomanticLevel(Hero.MainHero, Hero.OneToOneConversationHero) is not Romance.RomanceLevelEnum.FailedInPracticalities and
				not Romance.RomanceLevelEnum.FailedInCompatibility)
			{
				return false;
			}
