namespace Tweaks.Behaviors
{
	...
	internal class TweaksPregnancyCampaignBehavior : PregnancyCampaignBehavior
	{
		...
		{
			foreach (var logEntry in new List<LogEntry>(Campaign.Current.LogEntryHistory.GameActionLogs))
			{
		...
		private bool HeroPregnancyCheckCondition(Hero hero) => hero.IsFemale && hero.IsAlive && hero.Age >= Campaign.Current.Models.AgeModel.HeroComesOfAge && (hero.Clan == null || !hero.Clan.IsRebelClan) && !CampaignOptions.IsLifeDeathCycleDisabled;

		// Token: 0x06002FC3 RID: 12227 RVA: 0x000C9524 File Offset: 0x000C7724
		private void CheckOffspringsToDeliver(Hero hero)
		{
			var pregnancy = this._heroPregnancies.Find((TweaksPregnancyCampaignBehavior.Pregnancy x) => x.Mother == hero);
			if (pregnancy == null)
			{
		...
		{
			this.GetLocation(hero, out var settlement, out var mobileParty);
			this.GetLocation(spouse, out var settlement2, out var mobileParty2);
		...
		private void GetLocation(Hero hero, out Settlement heroSettlement, out MobileParty heroParty)
		{
			...
			{
				var mobileParty2 = heroParty;
				heroSettlement = mobileParty2?.CurrentSettlement;
			}
		}

		// Token: 0x06002FC7 RID: 12231 RVA: 0x000C962C File Offset: 0x000C782C
		private void CheckOffspringsToDeliver(TweaksPregnancyCampaignBehavior.Pregnancy pregnancy)
		{
			...
			if (!pregnancy.DueDate.IsFuture && pregnancy.Mother.IsAlive)
			{
				...
				for (var i = 0; i < num; i++)
				{
					if (MBRandom.RandomFloat > pregnancyModel.StillbirthProbability)
					{
						...
						var item = HeroCreator.DeliverOffSpring(mother, pregnancy.Father, isOffspringFemale, null);
						list.Add(item);
					}
					else
					{
						var textObject = new TextObject("{=pw4cUPEn}{MOTHER.LINK} has delivered stillborn.", null);
				...
				this.OnGivenBirth(mother, list, num2);
		...
		private void ChildConceived(Hero mother) => this._heroPregnancies.Add(new TweaksPregnancyCampaignBehavior.Pregnancy(mother, mother.Spouse, CampaignTime.DaysFromNow(Campaign.Current.Models.PregnancyModel.PregnancyDurationInDays)));

		// Token: 0x06002FCB RID: 12235 RVA: 0x000C97F8 File Offset: 0x000C79F8
		private void CreateYoungCharactersForFactions()
		{
			foreach (var hero2 in from hero in Hero.AllAliveHeroes.ToList<Hero>()
		...
		{
			var count = mother.Children.Count;
			var num = 48f;
			var num2 = mother.CharacterObject.Age - yearsAgo;
			foreach (var hero in mother.Children)
			{
		...
		public override void SyncData(IDataStore dataStore) => dataStore.SyncData<List<TweaksPregnancyCampaignBehavior.Pregnancy>>("_heroPregnancies", ref this._heroPregnancies);

		// Token: 0x04001007 RID: 4103
		private List<TweaksPregnancyCampaignBehavior.Pregnancy> _heroPregnancies = new List<TweaksPregnancyCampaignBehavior.Pregnancy>();
		...
		internal class Pregnancy
		{
			...
			internal static void AutoGeneratedStaticCollectObjectsPregnancy(object o, List<object> collectedObjects) => ((TweaksPregnancyCampaignBehavior.Pregnancy)o).AutoGeneratedInstanceCollectObjects(collectedObjects);
			...
			internal static object AutoGeneratedGetMemberValueMother(object o) => ((TweaksPregnancyCampaignBehavior.Pregnancy)o).Mother;

			// Token: 0x06004383 RID: 17283 RVA: 0x001215EE File Offset: 0x0011F7EE
			internal static object AutoGeneratedGetMemberValueFather(object o) => ((TweaksPregnancyCampaignBehavior.Pregnancy)o).Father;

			// Token: 0x06004384 RID: 17284 RVA: 0x001215FB File Offset: 0x0011F7FB
			internal static object AutoGeneratedGetMemberValueDueDate(object o) => ((TweaksPregnancyCampaignBehavior.Pregnancy)o).DueDate;
