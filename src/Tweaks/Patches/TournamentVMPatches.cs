...
namespace Tweaks.Patches
{
    ...
    public class RefreshValuesPatch
    {
        static void Postfix(TournamentVM __instance)
        {
            int num = TweaksMCMSettings.Instance is { } settings ? settings.TournamentMaxBetAmount : __instance.MaximumBetValue;
            if (Hero.MainHero.GetPerkValue(DefaultPerks.Roguery.DeepPockets))
            {
                num *= (int)DefaultPerks.Roguery.DeepPockets.PrimaryBonus;
            }
        ...
        static bool Prepare() => TweaksMCMSettings.Instance is { } settings && settings.TournamentMaxBetAmountTweakEnabled;
    }
