namespace Tweaks.Patches
{
	...
	public class BattleSizePatchEx_GetRealBattleSize
	{
		...
		private static bool Prepare() => TweaksMCMSettings.Instance is { } settings && settings.BattleSizeTweakExEnabled;
	}
	...
	internal class BattleSizePatchEx_BattleSizeSpawnTick
	{
		...
		public static float GetMountRatio() => BattleSizePatchEx_PartyGroupTroopSupplier.mountfootratio;
		...
		private static bool Prefix(MissionAgentSpawnLogic __instance)
		{
			...
			foreach (var agent in __instance.Mission.AllAgents)
			{
				if (agent.IsMount)
				{
					...
					{
						mountNoRider += 1;
						if (mountNoRider > Statics._settings.RetreatHorses)
						{
			...
			if (BattleSizePatchEx_PartyGroupTroopSupplier.isSiege == true)
			{
				BattleSizePatchEx_PartyGroupTroopSupplier.mountfootratio = 1f;
			}
