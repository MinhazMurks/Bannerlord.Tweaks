namespace KaosesPartySpeeds.Model
{
	...
	internal class TweaksPartySpeedCalculatingModel : DefaultPartySpeedCalculatingModel
	{
		...
		public override ExplainedNumber CalculateBaseSpeed(MobileParty mobileParty, bool includeDescriptions = false, int additionalTroopOnFootCount = 0, int additionalTroopOnHorseCount = 0)
		{
			...
			var num2 = 0f;
			var num3 = 0;
			var num4 = mobileParty.MemberRoster.TotalManCount + additionalTroopOnFootCount + additionalTroopOnHorseCount;
			AddCargoStats(mobileParty, ref num, ref num2, ref num3);
			var num5 = mobileParty.ItemRoster.TotalWeight;
			new ExplainedNumber(0f, false, null);
			var num6 = (int)Campaign.Current.Models.InventoryCapacityModel.CalculateInventoryCapacity(mobileParty, false, additionalTroopOnFootCount, additionalTroopOnHorseCount, 0, false).ResultNumber;
			var num7 = party.NumberOfMenWithHorse + additionalTroopOnHorseCount;
			var num8 = party.NumberOfMenWithoutHorse + additionalTroopOnFootCount;
			var num9 = party.MemberRoster.TotalWounded;
			var num10 = party.PrisonRoster.TotalManCount;
			var morale = mobileParty.Morale;
			...
			var baseNumber = this.CalculateBaseSpeedForParty(num4);
			var result = new ExplainedNumber(baseNumber, includeDescriptions, null);
			this.GetCavalryRatioModifier(mobileParty, num4, num7, ref result);
			this.GetFootmenPerkBonus(mobileParty, num4, num8, ref result);
			var num11 = Math.Min(num8, num);
			var mountedFootmenRatioModifier = this.GetMountedFootmenRatioModifier(num4, num11);
			...
			var num12 = Math.Min(num5, num6);
			if (num12 > 0f)
			{
				var cargoEffect = this.GetCargoEffect(num12, num6);
				result.AddFactor(cargoEffect, _textCargo);
			}
			if (num2 > num6)
			{
				var overBurdenedEffect = this.GetOverBurdenedEffect(num2 - num6, num6);
			...
			{
				var overPartySizeEffect = this.GetOverPartySizeEffect(mobileParty);
				result.AddFactor(overPartySizeEffect, _textOverPartySize);
			}
			num3 += Math.Max(0, num - num11);
			if (!mobileParty.IsVillager)
			{
				var herdingModifier = this.GetHerdingModifier(num4, num3);
			...
			var woundedModifier = this.GetWoundedModifier(num4, num9, mobileParty);
			...
			if (!mobileParty.IsCaravan)
			{
				...
				{
					var overPrisonerSizeEffect = this.GetOverPrisonerSizeEffect(mobileParty);
					result.AddFactor(overPrisonerSizeEffect, _textOverPrisonerSize);
				}
				var sizeModifierPrisoner = GetSizeModifierPrisoner(num4, num10);
			...
			{
				var playerMapMovementSpeedBonusMultiplier = Campaign.Current.Models.DifficultyModel.GetPlayerMapMovementSpeedBonusMultiplier();
		...
		{
			var itemRoster = mobileParty.ItemRoster;
			var numberOfPackAnimals = itemRoster.NumberOfPackAnimals;
			var numberOfLivestockAnimals = itemRoster.NumberOfLivestockAnimals;
		...
		private float CalculateBaseSpeedForParty(int menCount) => (float)(5.0 * Math.Pow(200f / (200f + menCount), 0.4000000059604645));
		...
		public override ExplainedNumber CalculateFinalSpeed(MobileParty mobileParty, ExplainedNumber finalSpeed)
		{
			...
			var effectiveScout = mobileParty.EffectiveScout;
			if (faceTerrainType == TerrainType.Forest)
			{
				...
				var num2 = (num / mobileParty.MemberRoster.Count > 0.75f) ? -0.15f : -0.3f;
				...
				{
					var value = DefaultCulturalFeats.BattanianForestSpeedFeat.EffectBonus * -num2;
					finalSpeed.AddFactor(value, _culture);
				}
			}
			else if (faceTerrainType is TerrainType.Water or TerrainType.River or TerrainType.Bridge or TerrainType.ShallowRiver)
			{
			...
			else if (faceTerrainType is TerrainType.Desert or TerrainType.Dune)
			{
			...
			if (effectiveScout != null)
			{
				...
				{
					var targetParty = mobileParty.TargetParty;
			...
			if ((army?.LeaderParty) != null && mobileParty.Army.LeaderParty != mobileParty && mobileParty.AttachedTo != mobileParty.Army.LeaderParty && mobileParty.Army.LeaderParty.HasPerk(DefaultPerks.Tactics.CallToArms, false))
			{
		...
		private float GetCargoEffect(float weightCarried, int partyCapacity) => -0.02f * weightCarried / partyCapacity;

		// Token: 0x06002E29 RID: 11817 RVA: 0x000C2A09 File Offset: 0x000C0C09
		private float GetOverBurdenedEffect(float totalWeightCarried, int partyCapacity) => -0.4f * (totalWeightCarried / partyCapacity);
		...
		{
			var partySizeLimit = mobileParty.Party.PartySizeLimit;
			var numberOfAllMembers = mobileParty.Party.NumberOfAllMembers;
		...
		private float GetOverPrisonerSizeEffect(MobileParty mobileParty)
		{
			...
			var numberOfPrisoners = mobileParty.Party.NumberOfPrisoners;
		...
		private float GetWoundedModifier(int totalMenCount, int numWounded, MobileParty party)
		{
			...
			var baseNumber = Math.Max(-0.8f, -0.05f * numWounded / totalMenCount);
			var explainedNumber = new ExplainedNumber(baseNumber, false, null);
		...
		private void GetCavalryRatioModifier(MobileParty party, int totalMenCount, int totalCavalryCount, ref ExplainedNumber result)
		{
			...
			{
				var value = 0.4f * totalCavalryCount / totalMenCount;
		...
		private void GetFootmenPerkBonus(MobileParty party, int totalMenCount, int totalFootmenCount, ref ExplainedNumber result)
		{
			...
			var num = totalFootmenCount / (float)totalMenCount;
		...
		private static float GetSizeModifierWounded(int totalMenCount, int totalWoundedMenCount) => (float)Math.Pow((10f + totalMenCount) / (10f + totalMenCount - totalWoundedMenCount), 0.33000001311302185);

		// Token: 0x06002E32 RID: 11826 RVA: 0x000C2B94 File Offset: 0x000C0D94
		private static float GetSizeModifierPrisoner(int totalMenCount, int totalPrisonerCount) => (float)Math.Pow((10f + totalMenCount + totalPrisonerCount) / (10f + totalMenCount), 0.33000001311302185);
