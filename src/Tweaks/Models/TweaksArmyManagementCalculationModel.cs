namespace Tweaks.Models
{
	...
	internal class TweaksArmyManagementCalculationModel : DefaultArmyManagementCalculationModel
	{
		...
		public override ExplainedNumber CalculateDailyCohesionChange(Army army, bool includeDescriptions = false)
		{
			...
			var result = new ExplainedNumber(baseChange, includeDescriptions, null);
			this.KaosesCalculateCohesionChangeInternal(army, IsClanOnlyarmy, ref result);
			//~ KT
			var leaderParty = army.LeaderParty;
			var siegeEvent = leaderParty?.SiegeEvent;
		...
		{
			var starvingParties = 0;
			var lowMoraleParties = 0;
			var lowHealthyTroopsParties = 0;
			var num4 = 0;
			var armyIsClanOnly = IsClanOnly;
		...
		{
			var result = true;
			var armyClan = army.LeaderParty.ActualClan;
			foreach (var mobileParty in army.Parties)
			{
				if (mobileParty != army.LeaderParty)
				{
		...
		public override int CalculateNewCohesion(Army army, PartyBase newParty, int calculatedCohesion, int sign)
		{
			...
			sign = Math.Sign(sign);
			var num = (sign == 1) ? (army.Parties.Count - 1) : army.Parties.Count;
			var num2 = MathF.Ceiling((float)((calculatedCohesion * num) + (100 * sign))) / (num + sign);
			if (num2 > 100)
			{
				return 100;
			}
		...
		public override int GetCohesionBoostInfluenceCost(Army army, int percentageToBoost = 100) => this.CalculateTotalInfluenceCost(army, percentageToBoost);

		// Token: 0x06002D74 RID: 11636 RVA: 0x000B5A32 File Offset: 0x000B3C32
		public override int GetCohesionBoostGoldCost(Army army, float percentageToBoost = 100f) => this.CalculateTotalInfluenceCost(army, percentageToBoost) * 40;
		...
		public override bool CheckPartyEligibility(MobileParty party) => party.Army == null && this.GetPartySizeScore(party) > this._minimumPartySizeScoreNeeded && party.MapEvent == null;
		...
		private readonly float _minimumPartySizeScoreNeeded = 0.4f;
