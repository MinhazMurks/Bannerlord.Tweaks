namespace Tweaks.Models
{
	...
	internal class TweaksBattleRewardModel : DefaultBattleRewardModel
	{
		...
		public override int GetPlayerGainedRelationAmount(MapEvent mapEvent, Hero hero)
		{
			...
			//~ KT
			double relationShipGain = this.GetPlayerGainedRelationAmount(0.75 + Math.Pow(playerPartyContributionRate * 1.3f * (num2 + renownValue), 0.6700000166893005));
		...
		{
			//~ KT
			var renowGainBase = 0.0f;
			renowGainBase = this.GetModifiedRenownGain(renownValueOfBattle * contributionShare);
			//~ KT
			//ExplainedNumber result = new ExplainedNumber(renownValueOfBattle * contributionShare, true, null);
			var result = new ExplainedNumber(renowGainBase, true, null);
			if (party.IsMobile)
			{
				...
				PerkHelper.AddPerkBonusForCharacter(famousCommander, mobileParty?.LeaderHero.CharacterObject, true, ref result);
			}
			return result;
		}
		...
		{
			//~ KT
			var influenceGainBase = 0.0f;
			influenceGainBase = this.GetModifiedInfluenceGain(party, influenceValueOfBattle * contributionShare);
			var result = new ExplainedNumber(party.MapFaction.IsKingdomFaction ? influenceGainBase : 0f, true, null);
		...
		{
			//~ KT
			var moraleGainBase = 0.0f;
			moraleGainBase = this.GetModifiedMoraleGain((float)(0.5f + (renownValueOfBattle * contributionShare * 0.5f)));
			...
			var result = new ExplainedNumber(moraleGainBase, true, null);
		...
		{
			var num = partyLeaderHero.Gold * 0.05f;
			if (num > 10000f)
			{
				num = 10000f;
			}
			//~ KT
			num = this.GetModifiedGoldLossAfterDefeat(num);
			//~ KT
			return (int)num;
		}
